...
class DownloadPage extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState(data, () => {
      this.getFile(), this.requestStorage();
    });
  }
  ...
  sendWa() {
    ...
    const descs = this.state.files[0].url.replace(
      '35.198.219.220',
      'pdf.ifca.co.id',
    );
  ...
  sendEmail() {
    ...
    const link = this.state.files[0].url.replace(
      '35.198.219.220',
      'pdf.ifca.co.id',
    );
    const message =
      "<!DOCTYPE html> <meta http-equiv='Content-Type' content='text/html; charset=iso-8859-1'> <body> <div>  Silakan download brosur pada link di bawah ini <br><br> " +
      link +
      '</div> </body> </html>';
    // alert(email_add);
      ...
      <Container style={Style.bgMain}>
        <Header style={Style.navigation}>
          ...
          <View style={Style.actionBarMiddle}>
            <Text style={Style.actionBarText}>{'Downloads'.toUpperCase()}</Text>
          </View>
            ...
            <ScrollView>
              ...
              <View style={Styles.overview}>
                <Image
                  style={[
                    Styles.picWidth,
                    {height: 190, borderRadius: 10, marginTop: 5},
                  ]}
