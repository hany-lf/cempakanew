...
class AttendancePage extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState(data, () => 
                  this.getTime(),
                  this.getLocationDefault()); //untuk load time
      ...
      .then((res) => {
        ...
        this.setState({
          //  currentTime: hour + ':' + minutes + ':' + seconds  ,
           day_server: moment(this.state.dataTime).format(
            "dddd"
          ),
          time_server: moment(this.state.dataTime).format(
            "MMMM DD YYYY"
          ),
          nowTime_server: moment(this.state.dataTime).format(
            "H:mm:ss"
          ),
          });
        }
    ...
    fetch(urlApi + "c_attendance/getLocationDefault", {
      ...
      .then((res) => {
        if (!res.Error) {
          ...
          this.setState({ location: resData.location_default, default_location: resData });
      ...
      await fetch(
        ...
        .then((res) => {
          ...
          if (res.status == "OK") {
            ...
            //});
            this.setState({ address, isLoaded: !this.state.isLoaded, isSuccess: true }, () => {
              messageToast = "Your address " + address;
            });
          ...
          <Content
            ...
            >
              {!this.state.isLoaded ? (
                ...
                <View
                  style={{ alignItems: "center", justifyContent: "center" }}
                >
                  <Text style={Style.textshow}>{this.state.default_location.location_default}</Text>
                </View>
              )}
            </View>
            ...
            {isSuccess ? (
              ...
              <TouchableOpacity
                style={[
                  Styles.wrapImg,
                  { borderWidth: 1, borderColor: "#888888" },
                ]}
                disabled={true}
                //onPress={this.getGeo}
              >
            ...
            <View style={Styles.wrapBtn}>
              <Button
                ...
                large
                rounded
                disabled={isSuccess == true && isFotoAv == true && isLoadedSubmit == true ? false : true}
